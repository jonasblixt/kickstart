...
int main(int argc, char **argv)
{
   ...
   int nl_socket;
    ...
    while (1) {
        ...
        {
            //printf("uevt: %i %s\n", r, msg);
            if (strcmp (msg, "bind@/devices/platform/bus@5b000000/5b010000.mmc/mmc_host/mmc0/mmc0:0001") == 0)
                break;
        }
    }
    ...
    printf ("Calling crypt_init_data_device\n");
    rc = crypt_init_data_device(&cd, "/dev/mmcblk0p3", "/dev/mmcblk0p3");

    printf ("crypt_init_data_device = %i\n",rc);
    params.flags = 0;
	...
	rc = crypt_activate_by_volume_key(cd, "vroot",
					 ...
					 hash_size,
					 activate_flags);
    printf ("crypt_activate_by_volume = %i\n",rc);
/*
    while (1) {
        int r = recv(nl_socket, msg, sizeof(msg), MSG_DONTWAIT);
        if (r >= 0)
